...
5. **出片** — `generateSeedanceVideo({ prompt:<视频提示词>, referenceImages:[产品图URL, 分镜图URL…], aspectRatio:"9:16", duration, generateAudio })`。
6. **取结果** — 超时拿 `taskToken` → `getSeedanceVideoResult` 轮询 → 视频 URL 用 markdown 发给用户。
